...
public class PlayerController : MonoBehaviour
{
    ...
    private float verticalVeolovity;                            //수평이동의 속도값 정의
    ...
    void Update()
    {
        ...
        //7. 기본 중력 설정
        if(controller.isGrounded && verticalVeolovity < 0f)
        {
            verticalVeolovity = -2f;
        }
        else
        {
            verticalVeolovity += gravity * Time.deltaTime;
        }

        controller.Move(Vector3.up * verticalVeolovity * Time.deltaTime);              //컨트롤러에 이동 방향과 속도를 준다.
